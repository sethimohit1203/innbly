        ...
        <div className="mx-auto grid h-18 max-w-7xl grid-cols-[1fr_auto_1fr] items-center px-6 sm:px-8 lg:px-10">
          ...
          <div className="flex items-center gap-3 justify-self-end">
            ...
            >
              ...
              {savedIds.length > 0 && (
                <span className="absolute right-0 top-0 flex h-4 min-w-4 items-center justify-center rounded-full bg-rose-600 px-1 text-[9px] font-bold text-white">
                  {savedIds.length}
                </span>
              )}
            </Link>

            {/* List Your Property Red Pill Button */}
            <button
              type="button"
            ...
            {isHost && (
              <button
                ...
                className="relative flex h-9 w-9 items-center justify-center rounded-full hover:bg-slate-50 text-slate-750 active:scale-95 transition"
              >
                <Bell className="h-4.5 w-4.5 text-slate-800" />
                {newBookingsCount > 0 && (
                  <span className="absolute right-0.5 top-0.5 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-rose-500 px-1 text-[8px] font-extrabold text-white ring-2 ring-white">
                    {newBookingsCount}
                  </span>
                )}
              </button>
            ...
            {user ? (
              <div className="relative">
                ...
                {avatarOpen && (
                  <>
                    ...
                    <div className="absolute right-0 top-full z-20 mt-2 w-52 rounded-xl border border-slate-200 bg-white p-2 shadow-card-hover text-left">
                      {isHost ? (
                        <>
                          <button
                            onClick={() => {
                            ...
                            className="w-full flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50 transition text-left"
                          >
                        ...
                        <>
                          <Link
                            ...
                            className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50"
                          >
                            <User className="h-4 w-4 text-slate-450" /> My Profile
                          </Link>
                          <Link
                            to="/bookings"
                            onClick={() => setAvatarOpen(false)}
                            className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50"
                          >
                            <Receipt className="h-4 w-4 text-slate-450" /> My Bookings
                          </Link>
                          <Link
                            to="/saved"
                            onClick={() => setAvatarOpen(false)}
                            className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50"
                          >
                            <Heart className="h-4 w-4 text-slate-450" /> Saved Properties
                          </Link>
                          <Link
                            to="/invite"
                            onClick={() => setAvatarOpen(false)}
                            className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50"
                          >
                            <Gift className="h-4 w-4 text-slate-450" /> Invite Friends
                          </Link>
                          ...
                          <button
                            ...
                            className="w-full flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50 transition text-left"
                          >
                            <Repeat className="h-4 w-4 text-slate-450" /> Switch to hosting
                          </button>
                        </>
                      )}
                      <div className="my-1 h-px bg-slate-100" />
                      <button
                        ...
                        className="w-full flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-bold text-rose-600 hover:bg-rose-50 transition text-left"
                      >
              ...
              <Button
                ...
                className="hidden hover:border-primary-400 hover:text-primary-700 sm:inline-flex text-xs font-bold rounded-full"
              >
