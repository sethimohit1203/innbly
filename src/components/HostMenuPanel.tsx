...
export function HostMenuPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      {/* Backdrop overlay */}
      <div className="fixed inset-0 z-50 bg-slate-900/30 backdrop-blur-sm transition-opacity duration-300 animate-fade-in" onClick={onClose} />

      {/* Slide-out Menu Drawer */}
      <div className="fixed inset-y-0 right-0 z-[51] w-full max-w-[340px] bg-white p-5 shadow-2xl transition-all duration-300 animate-slide-in flex flex-col justify-between">
        ...
        <div className="overflow-y-auto flex-1 pr-1 scrollbar-thin">
          ...
          <div className="flex items-center justify-between border-b border-slate-100 pb-5 mb-5 relative">
            ...
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-xl bg-rose-50/50 border border-rose-100 text-primary-600 hover:bg-rose-100/50 transition active:scale-95"
            >
          ...
          <div className="rounded-2xl bg-gradient-to-br from-rose-50/70 via-red-50/40 to-amber-50/40 border border-rose-100 p-4 relative overflow-hidden shadow-sm mb-6 text-left">
            ...
            <button 
              onClick={() => go('/profile')}
              className="mt-2.5 flex items-center gap-1 text-[10px] font-extrabold text-primary-600 hover:underline"
            >
              Get Started Now <ArrowRight className="h-3 w-3" />
            </button>
            
            {/* House and Trees Graphic Outline overlay */}
            <div className="absolute -bottom-1 -right-2 text-primary-600 opacity-15 pointer-events-none flex items-end gap-1">
        ...
        <div className="pt-4 border-t border-slate-100">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white py-3 text-xs font-bold text-slate-700 hover:border-slate-300 transition active:scale-95 shadow-sm"
          >
