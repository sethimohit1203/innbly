...
export const hostFormSchema = z.object({
  ...
  discountNewListing: z.boolean(),
  discountLastMinute: z.boolean(),
  discountWeekly: z.boolean(),
  discountMonthly: z.boolean(),

  safetyCamera: z.boolean(),
  safetyNoiseMonitor: z.boolean(),
  safetyWeapons: z.boolean(),
...
export async function submitHostListing(values: HostFormValues): Promise<HostSubmissionResult> {
  ...
  const { error } = await supabase.from('host_submissions').insert({
    ...
    discount_new_listing: values.discountNewListing,
    discount_last_minute: values.discountLastMinute,
    discount_weekly: values.discountWeekly,
    discount_monthly: values.discountMonthly,
    safety_camera: values.safetyCamera,
    safety_noise_monitor: values.safetyNoiseMonitor,
    safety_weapons: values.safetyWeapons,
